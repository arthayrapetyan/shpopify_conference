...
var App = function () {
    ...
    (0, _createClass3.default)(App, [{
        ...
        key: 'renderCard',
        value: function renderCard(e, isFirst) {
            var card = this.createNodeElementFromHTML(document.getElementById('user_info_card').innerHTML);
        ...
        value: function handleSubmit() {
            ...
            user_card.each(function (index, elem) {
                var role = index == 0 ? 1 : 0;
                var name = elem.querySelector('.name').value;
                var phone = elem.querySelector('.phone').value;

                if (!name || !phone) {
                    var classPrefix = !name ? 'name' : 'phone';
                    user_card.find('.' + classPrefix + '_required').removeClass('hide');
                    return;
                }

                participants.push({ name: name, phone: phone, role: role });
            });
        ...
        value: function handleAutocomplete() {
            $.ajax({
                url: '/customers',
                method: 'GET'
            }).then(function (res) {
                var customers = JSON.parse(res).customers;
                var customersData = $.map(customers, function (value, key) {
                    return {
                        id: value.id,
        ...
        value: function handleInputChange() {
            ...
            var requiredElem = $(this).parent().find('.' + inputName + '_required');
            var invalidElem = $(this).parent().find('.' + inputName + '_invalid');
        ...
        key: 'createNodeElementFromHTML',
        value: function createNodeElementFromHTML(htmlString) {
